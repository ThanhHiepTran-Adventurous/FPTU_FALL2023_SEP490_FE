  ...
  <dropdown text="Bottom">
    <template #trigger>
      <div class="flex items-center rounded-lg border">
        <div
          class="grid grid-cols-2 justify-items-stretch bg-[#F7F8FA] px-6 py-2 rounded-md !text-black"
          :class="props.class">
          <div class="w-max text-blue-600 font-bold">{{ selectedObject?.label }}</div>
    ...
    <list-group class="cursor-pointer w-min max-h-48 overflow-y-auto" :class="props.class">
      <list-group-item
        ...
        class="flex items-center gap-3 py-1 hover:bg-blue-50"
        @click="() => (selectedObject = item)">
        <div>
            <img :src="item.img" alt="selection-images" />
        </div>
        <div>
            {{ item.label }}
        </div>
