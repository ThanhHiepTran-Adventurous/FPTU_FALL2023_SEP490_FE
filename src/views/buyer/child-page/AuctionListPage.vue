...
// Create a computed property for paginated auctions
const paginatedAuctions = computed(() => {
  const startIndex = (currentPage.value - 1) * itemsPerPage
  const endIndex = startIndex + itemsPerPage
  return filteredAuctions.value.slice(startIndex, endIndex)
})

const filteredAuctions = computed(() => {
  // Filter by selected brands
  const selectedBrands = brandOptions.value.filter(brand => brand.isSelected).map(brand => brand.value)
  const brandFilteredAuctions =
    selectedBrands.length > 0
      ? auctions.value.filter(auction => selectedBrands.includes(auction.product.brand.id))
      : auctions.value
  ...
  const categoryFilteredAuctions =
    selectedCategories.length > 0
      ? auctions.value.filter(auction => selectedCategories.includes(auction.product.category.id))
      : auctions.value

  // Filter by price
  const priceFilteredAuctions = auctions.value.filter(auction => {
    const price = auction.highestPrice || auction.startPrice
    return price >= priceSelected.value[0] && price <= priceSelected.value[1]
  })

  // Combine all filters
  const filteredByBrandAndCategory = brandFilteredAuctions.filter(auction => categoryFilteredAuctions.includes(auction))
  return filteredByBrandAndCategory.filter(auction => priceFilteredAuctions.includes(auction))
})
</script>
<template>
  <div class="w-full">
